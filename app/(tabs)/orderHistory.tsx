    ...
    const fetchTransactionHistory = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          deleteOldTransactions(purchasedSnapshot.docs),
        ]);
        ...
        // YBC Reward Orders (from orders-history)
        // Separate reward-history collection
        rewardsSnapshot.forEach((doc) => {
          ...
          transactionsData.push({
            ...
            status: "completed",
            descriptions: [
              `Reward from child purchase ${data.purchaseAmount ?? ""} RM`,
            ],
          });
          userIds.add(data.fromUserId);
        });

        // Purchased YBC snapshot
        purchasedSnapshot.forEach((doc) => {
          const data = doc.data();
          if (!processTransaction(doc, "purchasedYBC")) return;
          transactionsData.push({
            id: doc.id,
            fromUserId:data.fromUserId,
            toUserId: data.toUserId,
            ...
            type: "purchasedYBC",
            status: 'completed',
            descriptions: [`Purchased ${data.amount} YBC for ${data.paidCredit ?? data.amount} RM`],
          });
        });
        ...
        ordersSnapshot.forEach((doc) => {
          ...
          if (data.type === 'reward') {
            ...
            transactionsData.push({
              ...
              type: "reward-history",
              status: 'completed',
              descriptions: [`${t.reward_from_child_purchase || 'Reward from child purchase'} ${data.purchaseAmount} RM`],
            });
            userIds.add(data.fromUserId);
            return;
          }
          ...
          const transactionWithDetails: TransactionWithDetails = {
            ...
            otherUserName:
              transaction.fromUserId === currentUser.uid
                ? userMap.get(transaction.toUserId) || 'YBL'
                : userMap.get(transaction.fromUserId) || 'YBL',
